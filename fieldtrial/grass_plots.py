    ...
    if len(treatments)>0:
        ...
        if len(dates)>0:  # Only include dates if they are not an empty list
            ...
            fig.update_traces(
                ...
                hovertemplate="Accession: %{customdata[0]}<br>Raw value: %{customdata[1]}<br>Plot ID: %{customdata[2]} (row: %{y},column: %{x})<br>Treatment: %{customdata[3]}<br>Date: %{customdata[4]}<extra></extra>"
            )
        else:
            fig.update_traces(
                customdata=np.moveaxis([accession, s_matrix, plotID, treatments], 0, -1),
                hovertemplate="Accession: %{customdata[0]}<br>Raw value: %{customdata[1]}<br>Plot ID: %{customdata[2]} (row: %{y},column: %{x})<br>Treatment: %{customdata[3]}<extra></extra>"
            )
        ...
        if len(dates)>0:  # Only include dates if they are not an empty list
            ...
            fig.update_traces(
                ...
                hovertemplate="Accession: %{customdata[0]}<br>Raw value: %{customdata[1]}<br>Plot ID: %{customdata[2]} (row: %{y},column: %{x})<br>Date: %{customdata[3]}<extra></extra>"
            )
        else:
            fig.update_traces(
                customdata=np.moveaxis([accession, s_matrix, plotID], 0, -1),
                hovertemplate="Accession: %{customdata[0]}<br>Raw value: %{customdata[1]}<br>Plot ID: %{customdata[2]} (row: %{y},column: %{x})<extra></extra>"
            )
